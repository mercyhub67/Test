...
if WindUI then
    ...
    -- ── ซ่อนปุ่ม toggle บน topbar ของ WindUI ────────────────────
    task.defer(function()
        for _, v in ipairs(game:GetService("Players").LocalPlayer.PlayerGui:GetDescendants()) do
            if v:IsA("ImageButton") and v.Name == "Toggle" then
                v.Visible = false
            end
        end
    end)

    -- ── Custom Toggle Button (เล็กลง + ลากได้) ───────────────────
    local _tgGui = Instance.new("ScreenGui")
    _tgGui.Name = "MercyToggleGui"
    _tgGui.ResetOnSpawn = false
    _tgGui.IgnoreGuiInset = true
    _tgGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    _tgGui.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")

    local _btn = Instance.new("ImageButton")
    _btn.Size = UDim2.new(0, 38, 0, 38)
    _btn.Position = UDim2.new(0, 12, 0.5, -19)
    _btn.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    _btn.BackgroundTransparency = 0.15
    _btn.BorderSizePixel = 0
    _btn.Image = "http://www.roblox.com/asset/?id=133253457738939"
    _btn.Parent = _tgGui
    ...
    -- Drag logic
    local _dragging, _dragStart, _startPos, _moved = false, nil, nil, false
    _btn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            _dragging = true
            _moved = false
            _dragStart = input.Position
            _startPos = _btn.Position
        end
    end)
    _btn.InputChanged:Connect(function(input)
        if _dragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
            local delta = input.Position - _dragStart
            if math.abs(delta.X) > 4 or math.abs(delta.Y) > 4 then _moved = true end
    ...
    _btn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            _dragging = false
        end
    end)
        ...
        if fovCircle then
            if isMobile then
                ...
                for i = 1, fovCircle._segments do
                    ...
                    fovCircle._lines[i].Color = Color3.fromHSV(((i / fovCircle._segments) + t * 0.25) % 1, 1, 1)
