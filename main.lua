...
-- ============================================================
print("[MercyHub] Script เริ่มรัน...")
...
-- ══════════════════════════════════════════════════════════════
local WindUI
local _windOk, _windErr = pcall(function()
    WindUI = loadstring(game:HttpGet(
        "https://github.com/Footagesus/WindUI/releases/latest/download/main.lua"
    ))()
end)
if not _windOk then
    warn("[MercyHub] WindUI โหลดไม่ได้: " .. tostring(_windErr))
else
    print("[MercyHub] WindUI โหลดสำเร็จ ✅")
end
...
if WindUI then
    ...
    local _stub = {}
    local _mt = { __index = function() return function() return _stub end end }
    setmetatable(_stub, _mt)
    Window = {
        Tab = function() return _stub end,
        ConfigManager = {
...
local ConfigManager = Window.ConfigManager or { CreateConfig = function()
    return { Register = function() end, Save = function() end, Load = function() end, Delete = function() end }
end }
    ...
    pcall(function()
        ...
        if fovCircle then
            if isMobile then
                ...
                local Center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
                fovCircle._hueOffset = (fovCircle._hueOffset + 0.005) % 1
                for i = 1, fovCircle._segments do
                    ...
                    fovCircle._lines[i].Color = Color3.fromHSV((i / fovCircle._segments + fovCircle._hueOffset) % 1, 1, 1)
...
RunService.Heartbeat:Connect(function()
    ...
    for _, tool in ipairs(char:GetChildren()) do
        if tool:IsA("Tool") and isGunTool(tool) then
            -- fire_rate apply ตลอดเวลาไม่ว่า Automatic จะเปิดหรือปิด
            pcall(function()
                tool:SetAttribute("fire_rate", getEnv().FireRateValue)
            end)
...
PlayerTab:Divider()
PlayerTab:Section({Title = "BRING PART"})
...
local function _getBringPlayer(name)
    name = string.lower(name)
    for _, p in ipairs(Players:GetPlayers()) do
        if string.find(string.lower(p.Name), name) or string.find(string.lower(p.DisplayName), name) then
            return p
...
local _bringNameInput       = ""
local _bringActive          = false
local _bringConnection      = nil
local _selectedBringPlayer2 = ""

local function toggleBringPart(playerName, state)
    _bringActive = state
    if _bringConnection then _bringConnection:Disconnect(); _bringConnection = nil end
    if not state then return end

    local target = _getBringPlayer(playerName)
    if not target then
        game:GetService("StarterGui"):SetCore("SendNotification", {
    ...
    for _, v in ipairs(workspace:GetDescendants()) do pcall(_forcePart, v) end
    _bringConnection = workspace.DescendantAdded:Connect(function(v) pcall(_forcePart, v) end)

    task.spawn(function()
        while _bringActive do
            if targetRoot and targetRoot.Parent then
                _bringAttachment1.WorldCFrame = targetRoot.CFrame
            end
            task.wait()
...
local _bringDropdown2
local _bringDropdown2 = PlayerTab:Dropdown({
    Title    = "เลือกผู้เล่น",
    Values   = getPlayerList(),
    Value    = getPlayerList()[1] or "ไม่มีผู้เล่นอื่น",
    Multi    = false,
    Callback = function(selected)
        if type(selected) == "string" and selected ~= "ไม่มีผู้เล่นอื่น" then
            _selectedBringPlayer2 = selected
        else
            _selectedBringPlayer2 = ""
        end
    end,
})
...
PlayerTab:Button({
    ...
    Callback = function()
        ...
        pcall(function() _bringDropdown2:Refresh(names, true) end)
        game:GetService("StarterGui"):SetCore("SendNotification", {
...
local _bringToggle2
_bringToggle2 = PlayerTab:Toggle({
    Title = "Bring Part (ค้างไว้)",
    Desc  = "ดึง Part ทุกอันในแมพเข้าหาผู้เล่นที่เลือก",
    Value = false,
    Callback = function(state)
        if state then
            if not _selectedBringPlayer2 or _selectedBringPlayer2 == "" then
                game:GetService("StarterGui"):SetCore("SendNotification", {
                    Title = "Bring", Text = "กรุณาเลือกผู้เล่นก่อน", Duration = 2
                })
                _bringToggle2:Set(false)
                return
            end
            toggleBringPart(_selectedBringPlayer2, true)
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Bring", Text = "กำลังดึง Part ใส่ " .. _selectedBringPlayer2, Duration = 2
            })
        else
            toggleBringPart(_selectedBringPlayer2, false)
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Bring", Text = "หยุดดึง", Duration = 2
            })
        end
    end,
})
...
Players.PlayerAdded:Connect(function()
    task.wait(0.5)
    pcall(function() _bringDropdown2:Refresh(getPlayerList(), true) end)
end)
Players.PlayerRemoving:Connect(function(player)
    task.wait(0.5)
    pcall(function() _bringDropdown2:Refresh(getPlayerList(), true) end)
    if _bringActive and _selectedBringPlayer2 == player.Name then
        _bringToggle2:Set(false)
        toggleBringPart(_selectedBringPlayer2, false)
        game:GetService("StarterGui"):SetCore("SendNotification", {
