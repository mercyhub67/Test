...
_btn.Parent = _gui
_btn.Size = UDim2.fromOffset(42, 42)
_btn.Position = UDim2.fromOffset(40, 220)
_btn.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
_btn.Image = ICON_ID
_btn.ScaleType = Enum.ScaleType.Fit
_btn.ImageTransparency = 0
_btn.AutoButtonColor = false
_btn.Active = true
_btn.Draggable = true
_btn.AnchorPoint = Vector2.new(0.5, 0.5)

local _corner = Instance.new("UICorner", _btn)
_corner.CornerRadius = UDim.new(0, 10)

local _stroke = Instance.new("UIStroke", _btn)
_stroke.Thickness = 1
_stroke.Color = Color3.fromRGB(180, 180, 180)

local _pad = Instance.new("UIPadding", _btn)
_pad.PaddingTop = UDim.new(0, 6)
_pad.PaddingBottom = UDim.new(0, 6)
_pad.PaddingLeft = UDim.new(0, 6)
_pad.PaddingRight = UDim.new(0, 6)
        ...
        if fovCircle then
            if isMobile then
                ...
                for i = 1, fovCircle._segments do
                    ...
                    fovCircle._lines[i].Color = Color3.fromHSV((hueOffset + i / fovCircle._segments) % 1, 1, 1)
...
local function isDowned()
    ...
    if hum:GetAttribute("HasBeenDowned") or hum:GetAttribute("IsDead") then return true end
    return false
end
...
local function teleportUnderground()
    ...
    underMapPos = root.Position + Vector3.new(0, -55, 0)
    root.CFrame = CFrame.new(underMapPos)
end
    ...
    task.spawn(function()
        while isFlickering and antiKillEnabled and isDowned() do
            ...
            if root and underMapPos then
                ...
                root.CFrame = CFrame.new(underMapPos + offset)
                task.wait(0.05)
                root.CFrame = CFrame.new(underMapPos)
            end
            task.wait(0.1)
...
RunService.Heartbeat:Connect(function()
    ...
    if isDowned() then
        ...
        if underMapPos then
            local root = getHRP()
            if root then root.CFrame = CFrame.new(underMapPos + Vector3.new(0, 55, 0)) end
        end
        underMapPos = nil
        isFlickering = false
    end
