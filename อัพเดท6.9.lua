...
local AntiAimTrack       = nil
local AntiAimbotAnim     = nil
local AntiAimbotAnimId   = "rbxassetid://109873544976020"
...
pcall(function() CombatTab:Section({ Title = "ANTI AIM:" }) end)

-- ── Anti-Aimbot Routine ───────────────────────────────────
task.spawn(function()
    while RunService.Heartbeat:Wait() do
        if not antiAimbotEnabled then
            if AntiAimbotAnim then
                AntiAimbotAnim:Stop()
                AntiAimbotAnim = nil
            end
            continue
        ...
        for _, track in next, animator:GetPlayingAnimationTracks() do
            if track.Animation.AnimationId == AntiAimbotAnimId then
                playing = true break
            end
        end
        if AntiAimbotAnim and not playing then AntiAimbotAnim = nil end
        if not playing then
            local anim = Instance.new("Animation")
            anim.AnimationId = AntiAimbotAnimId
            local track = animator:LoadAnimation(anim)
            AntiAimbotAnim = track
...
task.spawn(function()
    while true do
        ...
        for _, conn in next, getconnections(hum:GetPropertyChangedSignal("HipHeight")) do
            if conn.Function then conn:Disable() end
        end
        hum.HipHeight = 1
    end
end)
...
CombatTab:Toggle({
    Title    = "Anti Aim (Hitbox Sink)",
    Icon     = "shield",
    Default  = false,
    Callback = function(v)
        antiAimEnabled = v
        if not v and AntiAimTrack then
            AntiAimTrack:Stop()
            AntiAimTrack = nil
        end
    end,
...
CombatTab:Toggle({
    Title    = "Anti-Aimbot (Jitter)",
    Icon     = "zap",
    Default  = false,
    Callback = function(v)
        antiAimbotEnabled = v
        if not v and AntiAimbotAnim then
            AntiAimbotAnim:Stop()
            AntiAimbotAnim = nil
        end
    end,
