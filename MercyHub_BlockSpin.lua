    ...
    pcall(function()
        ...
        if fovCircle then
            if isMobile then
                ...
                local Center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
                local rainbowOffset = (tick() * 0.5) % 1
                for i = 1, fovCircle._segments do
                    ...
                    fovCircle._lines[i].Color = Color3.fromHSV((i / fovCircle._segments + rainbowOffset) % 1, 1, 1)
...
local function teleportUnderground()
    ...
    underMapPos = root.CFrame + Vector3.new(0, -25, 0)
